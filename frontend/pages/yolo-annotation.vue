<template>
  <div class="space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center justify-between">
      <div>
        <h1 class="font-bold text-lg text-dark">YOLO BoundingBox (Train Inference)</h1>
      ...
      <button :disabled="pipelineRunning"
        ...
        {{ pipelineRunning ? 'Training...' : 'Train YOLO' }}
      </button>
    </div>
    ...
    <div v-else-if="!filteredItems.length" class="bg-white rounded-xl p-10 text-center border border-gray-100">
      <p class="text-dark/50 text-sm">No YOLO predictions. Click "Train YOLO" above.</p>
    </div>
...
const filteredItems = computed(() => (data.value?.train_real || []).filter((img: any) => img.viz_url && img.predictions.length > 0))
