<template>
  <div class="space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center justify-between">
      <div>
        <h1 class="font-bold text-lg text-dark">Test Inference Result</h1>
        <p class="text-sm text-dark/50">{{ results.length }} annotated images</p>
      </div>
    ...
    <div v-else-if="!paginatedItems.length" class="bg-white rounded-xl p-10 text-center border border-gray-100">
      <p class="text-dark/50 text-sm">No test results. Click "Run All Test" above.</p>
    </div>
    ...
    <template v-else>
      ...
      <Pagination :page="page" :total="results.length" :per="perPage" @update:page="page = $event" />
    </template>
...
const results = computed(() => (data.value?.test_real || []).filter((img: any) => img.viz_url))
