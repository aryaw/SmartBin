    ...
    if progress_callback:
        def _on_epoch_end(trainer):
            progress_callback({
                ...
                "loss": round(float(sum(trainer.loss.values())), 4) if isinstance(trainer.loss, dict) else (round(float(trainer.loss), 4) if hasattr(trainer, "loss") and trainer.loss is not None else None),
            })
